...
func _task_bake(p_center: Vector3) -> void:
	...
	if terrain:
		...
		# Debug: Log face Y range
		if faces.size() >= 3 and log_timing:
			var min_y := INF
			var max_y := -INF
			for i in range(mini(faces.size(), 30)):
				min_y = minf(min_y, faces[i].y)
				max_y = maxf(max_y, faces[i].y)
			print("[NavBaker] Terrain faces: %d, Y range: %.2f to %.2f" % [faces_added, min_y, max_y])

	if source_geometry.has_data():
		NavigationServer3D.bake_from_source_geometry_data(nav_mesh, source_geometry)
		_bake_finished.call_deferred(nav_mesh, faces_added)
	else:
		_bake_finished.call_deferred(null, 0)


func _bake_finished(p_nav_mesh: NavigationMesh, p_faces_added: int = 0) -> void:
	if log_timing:
		print("[NavBaker] Bake took %.3fs, faces=%d" % [_bake_task_timer, p_faces_added])
	...
	if p_nav_mesh:
		...
		if log_timing:
			var verts := p_nav_mesh.get_vertices()
			if verts.size() > 0:
				var min_y := INF
				var max_y := -INF
				for v in verts:
				...
				print("[NavBaker] NavMesh %d polys, %d verts, Y: %.2f to %.2f" % [p_nav_mesh.get_polygon_count(), verts.size(), min_y, max_y])
