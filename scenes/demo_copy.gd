...
func _ready() -> void:
	...
	$RuntimeNavigationBaker.player = null  # Will set after captain spawns
	$RuntimeNavigationBaker.enabled = false  # Don't enable until captain exists

	# Spawn captain like demo spawns enemies - at Y=50, let gravity settle
	_spawn_captain()

	# Now enable nav baking
...
func _spawn_captain() -> void:
	# Use enemy_demo.gd pattern - spawn at Y=50, let gravity settle
	var captain_scene: PackedScene = preload("res://src/characters/captain.tscn")
	captain = captain_scene.instantiate()
	captain.name = "Captain"
	add_child(captain)

	# Spawn high like demo does, gravity will settle unit to terrain
	captain.global_position = Vector3(0, 50, 0)

	# Setup camera to follow captain
	_setup_camera()
